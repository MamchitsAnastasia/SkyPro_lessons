...
def test_invalid_api_response(mock_get):
    """Тестирует функцию convert_to_rub с неверным форматом данных от API"""
    mock_response = Mock()
    mock_response.json.return_value = {"error": "Invalid data"} #Имитирую возврат некорректных данных
    mock_response.raise_for_status.return_value = None #Имитирую успешный ответ
    mock_get.return_value = mock_response
