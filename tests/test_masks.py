...
def test_valid_card_numbers(generate_random_card_number):
    ...
    for card_number in valid_card_numbers:
        expected_output = card_number[:4] + " " + card_number[4:6] + "** **** " + card_number[-4:]
        assert masks.get_mask_card_number(card_number) == expected_output


def test_wrong_length_for_card_number(generate_random_card_number):
    """Тестирует функцию с номером карты неправильной длины."""
    wrong_lengths = [
        generate_random_card_number(length=15),  # Слишком короткий
        generate_random_card_number(length=17)  # Слишком длинный
    ]
...
def test_non_digit_for_card_number(generate_random_card_number):
...
def test_invalid_type_for_card_number(generate_random_card_number):
...
def test_valid_account_id(generate_random_account_id):
...
def test_wrong_length_account_number(generate_random_account_id):
    """Тестирует функцию с номером счетов неправильной длины."""
    wrong_lengths = [
        generate_random_account_id(length=19),  # Слишком короткий
        generate_random_account_id(length=21)  # Слишком длинный
    ]
...
def test_non_digit_for_account_id(generate_random_account_id):
...
def test_invalid_type_for_account_id(generate_random_account_id):
    """Тестирует функцию с неверным типом входного аргумента."""
    invalid_types = [
